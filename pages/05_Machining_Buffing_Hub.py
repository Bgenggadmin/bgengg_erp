...
# --- CONFIGURATION & HUB LOGIC (UPDATED TO MATCH MASTER SETUP SCRIPT) ---
if st.session_state.hub == "Machining Hub":
    # Updated table and column names to match your Master Setup audit
    ...
    # Assuming master_machines is used as station master for now
...
# Updated Master Table names to match your Admin Setup Page
OP_MASTER = "master_workers"
VN_MASTER = "master_clients"
VH_MASTER = "master_vehicles"

# 2. Data Fetching & Sanitization
def get_all_data():
    try:
        m_data = conn.table(MASTER_TABLE).select(MASTER_COL).execute().data or []
        # Column names updated to 'name' as per master_workers
        o_data = conn.table(OP_MASTER).select("name").execute().data or []
        # Column names updated to 'name' as per master_clients
        v_data = conn.table(VN_MASTER).select("name").execute().data or []
        
        vh_list = []
        if not IS_BUFFING:
            # Column name updated to 'reg_no' as per master_vehicles
            vh_data = conn.table(VH_MASTER).select("reg_no").execute().data or []
        ...
        return [r[MASTER_COL] for r in m_data], [o['name'] for o in o_data], \
               [v['name'] for v in v_data], vh_list, df
...
# --- TAB 4: MASTERS (ALIGNMENT COMPLETED) ---
with tabs[3]:
    st.markdown("### 🛠️ System Master Registry")
    master_options = {MASTER_TABLE: MASTER_COL, OP_MASTER: "name", VN_MASTER: "name"}
    if not IS_BUFFING:
        master_options[VH_MASTER] = "reg_no"
    selected_cat = st.segmented_control("Choose Registry to Manage", options=list(master_options.keys()),
        format_func=lambda x: x.replace('_', ' ').title(), default=MASTER_TABLE)
    st.divider()
    col_view, col_add = st.columns([2, 1], gap="large")
    with col_view:
        st.subheader(f"📋 Current Entries")
        try:
