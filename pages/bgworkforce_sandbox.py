...
with tabs[0]:
    ...
    if emp_summ_res:
        ...
        with sum_col_left:
            with st.expander(f"Work Logs ({len(work_summ_res)})", expanded=True):
                ...
                else: st.info("No work logged yet.")
        with sum_col_right:
            with st.expander(f"Movements ({len(move_summ_res)})", expanded=True):
                ...
                else: st.info("No movements recorded.")
    ...
    with col_a:
        ...
        if not att_data:
            ...
            log = att_data[0]
            p_in = pd.to_datetime(log['punch_in']).tz_convert(IST).time()
    ...
    if admin_pass == "bgadmin":
        ...
        with admin_tabs[0]:
            ...
            if t_att:
                ...
                c2.warning(f"📉 Low Work Logs (<4h)"); c2.dataframe(work_sums[work_sums['hours_spent'] < 4.0], hide_index=True)
                c3.success(f"🚀 High Work Logs (>7.5h)"); c3.dataframe(work_sums[work_sums['hours_spent'] > 7.5], hide_index=True)
            st.divider()
            if t_att:
                tdf = pd.DataFrame(t_att)
                def get_summary(row):
                    start = pd.to_datetime(row['punch_in']).tz_convert(IST)
                    end = (pd.to_datetime(row['punch_out']).tz_convert(IST) if pd.notnull(row['punch_out']) else datetime.now(IST))
        ...
        with admin_tabs[1]:
            ...
            elif l_type == "Attendance Timeline":
                ...
                for _, row in pend.iterrows():
                    with st.container(border=True):
                        ...
                        with c3.popover("❌ Reject"):
                            r_n = st.text_input("Reason", key=f"rn_{row['id']}")
                            if st.button("Confirm", key=f"rb_{row['id']}"):
                                conn.table("leave_requests").update({"status": "Rejected", "reject_reason": r_n}).eq("id", row['id']).execute(); st.rerun()
