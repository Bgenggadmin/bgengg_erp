...
with tabs[0]:
    ...
    if att_user == "Admin":
                            ...
                            with st.expander("✍️ Reply to Staff"):
                                with st.form(key=f"admin_rep_{r['id']}"):
                                    ...
                                    if st.form_submit_button("Send"):
                                        conn.table("founder_interaction").update({"reply_content": a_rep, "is_read": True}).eq("id", r['id']).execute()
                                        st.rerun()
                        st.divider()
        ...
        emp_msgs = conn.table("founder_interaction").select("*").or_(f"target_user.eq.{att_user},sender_name.eq.{att_user}").gte("created_at", f"{today}T00:00:00").order("created_at", desc=True).execute().data
        if emp_msgs:
            for m in emp_msgs:
                        ...
                        if not m.get('reply_content'):
                            with st.form(key=f"rep_form_{m['id']}", clear_on_submit=True):
                                ...
                                if st.form_submit_button("✔️ Submit"):
                                    conn.table("founder_interaction").update({"is_read": True, "reply_content": r_text or "Acknowledged", "replied_at": datetime.now(IST).isoformat()}).eq("id", m['id']).execute()
                                    st.rerun()
                        else:
        ...
        with st.expander("✉️ Send Update to Founder"):
            with st.form("new_emp_msg", clear_on_submit=True):
                ...
                if st.form_submit_button("🚀 Send"):
                    if new_msg:
                        conn.table("founder_interaction").insert({"sender_name": att_user, "content": new_msg, "target_user": "Admin", "is_read": False}).execute()
                        st.rerun()
    ...
    if show_shift_controls:
        if log_data and not log_data.get('punch_out'):
            ...
            if not st.session_state["promise_confirmed"]:
                with st.container(border=True):
                    st.markdown('<div style="background-color:#f8f9fb; padding:10px; border-left: 5px solid #007bff;"><b>"I am dedicated to B&G’s systems. Following the system today is my path to precision."</b></div>', unsafe_allow_html=True)
            ...
            if not emp_summ_res:
                ...
                if not log_data.get('punch_out'):
                    ...
                    if st.button("🏁 PUNCH OUT", use_container_width=True, type="primary"):
                        conn.table("attendance_logs").update({"punch_out": get_now_ist().isoformat(), "work_satisfaction": work_sat, "system_promise": st.session_state.get("promise_confirmed", False)}).eq("id", log_data['id']).execute()
        ...
        with cb:
            ...
            active_move_res = conn.table("movement_logs").select("*").eq("employee_name", att_user).is_("return_time", "null").execute().data
            active_move = [m for m in active_move_res if m['exit_time'][:10] == now_str] if active_move_res else []
            if not active_move:
                with st.form("move_out_form", clear_on_submit=True):
                    reason = st.selectbox("Category", ["Meeting", "Work Review", "Material", "Inspection", "Vendor Visit", "Lunch", "Personal"], key="selectbox_move_reason")
                    ...
                    if st.form_submit_button("📤 TIME OUT", use_container_width=True):
                        ...
                        else: st.error("Enter Destination")
            else:
                ...
                if st.button("📥 LOG TIME IN", use_container_width=True, type="primary", key="btn_move_in"):
                    conn.table("movement_logs").update({"return_time": get_now_ist().isoformat()}).eq("id", current['id']).execute(); st.rerun()
...
with tabs[1]:
    ...
    with h_col1:
        hist_type = st.radio("Select View", ["My Work Logs", "My Attendance History", "My Work Plans", "My Movements"], horizontal=True, key="hist_type_selector")
    ...
    if len(hist_range) == 2:
        ...
        mapping = {"My Work Logs": ("work_logs", "work_date"), "My Attendance History": ("attendance_logs", "work_date"), "My Work Plans": ("work_plans", "plan_date"), "My Movements": ("movement_logs", "exit_time")}
        ...
        if hist_res:
            ...
            for col in time_cols:
                if col in df_hist.columns:
                    df_hist[col] = pd.to_datetime(df_hist[col], errors='coerce').dt.tz_convert(IST).dt.strftime('%d-%m %I:%M %p')
            st.dataframe(df_hist, use_container_width=True, hide_index=True)
            st.download_button(label=f"📥 Download {hist_type} (CSV)", data=convert_df(df_hist), file_name=f"{att_user}_{hist_type.replace(' ', '_')}.csv", mime="text/csv")
        else: st.info(f"No records found for {hist_type} in this date range.")
...
with tabs[2]:
    ...
    with st.form("leave_form", clear_on_submit=True):
        ...
        c1, c2 = st.columns(2)
        sd, ed = c1.date_input("Start date", key="leave_sd"), c2.date_input("End date", key="leave_ed")
        reason_l = st.text_area("Reason for Leave")
        if st.form_submit_button("🚀 Submit Application", use_container_width=True):
            if ed < sd: st.error("❌ Error: End date cannot be before Start date.")
            elif not reason_l: st.warning("⚠️ Please provide a reason.")
            else:
                ...
                st.success("✅ Application Submitted!"); st.rerun()
    st.divider()
...
with tabs[3]:
    ...
    u_sel = st.selectbox("Check balance for:", get_staff_list(), index=get_staff_list().index(att_user) if att_user in get_staff_list() else 0, key="bal_u_final")
    if not df_l.empty:
        u_df = df_l[(df_l['employee_name'] == u_sel) & (df_l['status'] == 'Approved')].copy()
        if not u_df.empty:
            u_df['day_count'] = (pd.to_datetime(u_df['end_date']) - pd.to_datetime(u_df['start_date'])).dt.days + 1
            used = u_df['day_count'].sum()
        else: used = 0
        remaining = max(0, 12 - used)
        st.metric("Available Balance", f"{int(remaining)} Days", delta=f"{int(used)} Used", delta_color="inverse")
        st.progress(min(100, int((used / 12) * 100)) / 100)
    else: st.metric("Casual Leave Balance", "12 Days", "0 Used")
...
with tabs[4]:
    ...
    if admin_pass == "bgadmin":
        ...
        if export_mode == "Weekly": sr, er = date.today() - timedelta(days=7), date.today()
            ...
            sr, er = st.columns(2)[0].date_input("From"), st.columns(2)[1].date_input("To")
        ...
        with admin_tabs[4]:
            ...
            if st.button("Update Access Key"):
                conn.table("employee_auth").upsert({"employee_name": target_emp, "access_key": new_key}).execute(); st.success("Key updated!")
