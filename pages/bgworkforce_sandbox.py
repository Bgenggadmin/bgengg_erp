        ...
        with admin_tabs[0]:
            ...
            if t_att and t_work:
                ...
                with c1:
                    ...
                    fig_job = px.pie(job_dist, values='hours_spent', names='Job', hole=0.4, color_discrete_sequence=px.colors.qualitative.Pastel)
                    st.plotly_chart(fig_job, use_container_width=True)

                with c2:
                    st.markdown("##### ⚡ Output per Staff Member")
                    staff_perf = df_work_an.groupby('employee_name')['hours_spent'].sum().reset_index()
                    fig_perf = px.bar(staff_perf, x='employee_name', y='hours_spent', color='hours_spent', color_continuous_scale='Viridis')
