...
# 1. SETUP (set_page_config removed for sub-page compatibility)
...
with tab3:
    ...
    with col_cust:
        ...
        new_cust = st.text_input("New Customer Name", key="add_cust_input_master")
        if st.button("➕ Add Customer", key="add_cust_btn_master"):
            if new_cust:
                conn.table("customer_master").insert({"name": new_cust}).execute()
                st.rerun()
    with col_job:
        st.subheader("🔢 Job Codes")
        new_job = st.text_input("New Job Code", key="add_job_input_master")
        if st.button("➕ Add Job Code", key="add_job_btn_master"):
            if new_job:
