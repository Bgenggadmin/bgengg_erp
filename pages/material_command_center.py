...
def get_jobs():
    """Pulls unique job numbers from the projects master table"""
    try:
        res = conn.table("anchor_projects").select("job_no").execute()
        # Filter for non-null and non-empty jobs
        jobs = [str(r['job_no']).strip() for r in res.data if r.get('job_no')]
        return sorted(list(set(jobs)))
    except Exception as e:
        st.error(f"Error loading Job Codes: {e}")
        return []
...
with main_tabs[0]:
    ...
    with st.expander("➕ Add Item to this Indent", expanded=True):
        with st.form("indent_item_form", clear_on_submit=True):
            f1, f2 = st.columns(2)
            # Pulling from Master Jobs List
            job_list = get_jobs()
            selected_jobs = f1.multiselect("Target Job Nos", job_list)
            ...
            # TRIGGER LOGIC: Item-wise Trigger available in form
    ...
    # PART C: SEARCH HISTORY (REVISED TO DROP DOWN)
    st.divider()
    st.subheader("🔍 Export Indent Form & History")
    
    # Updated Filter Logic: Dropdown instead of typing
    available_jobs_in_history = ["ALL"] + get_jobs()
    search_j = st.selectbox("Filter History by Job Code", options=available_jobs_in_history)
    ...
    if hist_res.data:
        ...
        st.download_button("📥 Download Filtered Indent (CSV)", data=csv, file_name=f"BG_Indent_{search_j}_{date.today()}.csv")
    ...
    if res_p.data:
        ...
        for _, row in df_p.iterrows():
            ...
            with st.container(border=True):
                ...
                # LOGIC TO SEND TO SUPPLIERS
                share_text = f"B&G Enquiry:\nJob: {row['job_no']}\nItem: {row['item_name']}\nQty: {row['quantity']} {row.get('units')}\nSpecs: {row.get('specs')}"
                whatsapp_url = f"https://wa.me/?text={urllib.parse.quote(share_text)}"
                h3.markdown(f"[📲 Send to Supplier (WhatsApp)]({whatsapp_url})")
                ...
                with st.expander("🛠️ Finalize Purchase"):
                    ...
                    p_rem = st.text_input("Remarks / Vendor Name", key=f"rem_{row['id']}")
                    
                    if st.button("✅ Confirm Order", key=f"ok_{row['id']}", use_container_width=True, type="primary"):
                        conn.table("purchase_orders").update({"status": "Ordered", "po_no": p_no, "po_date": str(p_dt), "purchase_reply": p_rem}).eq("id", row['id']).execute()
                        st.rerun()

# --- TAB 3: STORES & TAB 4: MASTERS (Keep Existing Logic) ---
# [Code omitted for brevity, remains identical to previous version]
