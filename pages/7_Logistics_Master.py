...
# --- 3. UI LAYOUT (Tabs for Speed & Organization) ---
...
with tabs[0]:
    with st.form("request_form", clear_on_submit=True):
        ...
        reason = st.text_area("Purpose of Visit / Items to move")
        
        if st.form_submit_button("Submit Request"):
            if req_by and dest:
                new_req = {
                    "requested_by": req_by.upper(), "destination": dest.upper(), 
                    "req_date": str(r_date), "req_time": r_time, 
                    "purpose": reason, "status": "Pending"
                }
                conn.table("logistics_requests").insert(new_req).execute()
                
                # WhatsApp Notification Logic
                wa_msg = f"🚚 *New Vehicle Request*\nStaff: {req_by}\nTo: {dest}\nDate: {r_date}\nTime: {r_time}\nReason: {reason}"
                st.success("Request sent to Brahmiah!")
                st.link_button("📲 Ping Brahmiah on WhatsApp", get_wa_link("919848993939", wa_msg))
            else:
                st.error("Please fill in your name and destination.")
        ...
        for r in req_data:
            with st.expander(f"🚩 {r['requested_by']} to {r['destination']} ({r['req_date']})"):
                ...
                if st.button("Approve & Assign", key=f"btn{r['id']}"):
                    ...
                    st.success(f"{v_assign} Assigned!")
                    st.rerun()
    else:
        st.info("No pending requests at the moment.")

# --- TAB 3: TRIP LOGGER (Your Original Form) ---
with tabs[2]:
                ...
                if cam_photo:
                    try:
                        ...
                        conn.client.storage.from_("logistics-photos").upload(
                            path=photo_filename, file=buf.getvalue(),
                            file_options={"content-type": "image/jpeg"}
                        )
                    except: st.warning("Photo upload failed, saving log data only.")

                trip_dist = end_km - start_km if end_km > start_km else 0
                new_entry = {
                    "timestamp": datetime.now(IST).strftime('%Y-%m-%d %H:%M'),
                    "vehicle": vehicle, "driver": driver, "authorized_by": auth_by.upper(),
                    "start_km": start_km, "end_km": end_km, "distance": trip_dist,
                    "fuel_ltrs": fuel_qty, "fuel_rate": fuel_rate, "total_fuel_cost": total_fuel_cost,
                    "purpose": purpose, "location": location.upper(), "items": items.upper(), "photo_path": photo_filename
                }
