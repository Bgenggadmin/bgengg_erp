            ...
            if st.form_submit_button("Add to System", use_container_width=True):
                if new_head.strip():
                    ...
                    if new_head.upper() in [h.upper() for h in existing]:
                        ...
                        safe_db_write(
                            lambda: conn.table("petty_cash_heads").insert({
                                "head_name": new_head.strip().upper()
                            }).execute(),
                            success_msg=f"✅ '{new_head.upper()}' added.",
...
with tabs[5]:
    ...
    if not df_topups.empty:
        ...
        # Summary
        total = df_topups['amount'].sum() if 'amount' in df_topups.columns else 0
