    ...
    if f_job != "-- Select --":
        ...
        if not form_gates:
            ...
            with st.form("prod_form", clear_on_submit=True):
                ...
                # --- INTEGRATED MULTI-WORKER SELECTION ---
                ...
                if st.form_submit_button("🚀 Log Progress"):
                    if f_wrk_list:
                        # Combine list of workers into a comma-separated string for DB storage
                        ...
                        st.cache_data.clear(); st.success(f"Logged {len(f_wrk_list)} workers!"); st.rerun()
                    else:
...
# --- TAB 3: ANALYTICS (FULLY PRESERVED) ---
with tab_analytics:
    ...
    if not df_logs.empty:
        ...
        with v1:
            ...
            st.plotly_chart(px.bar(job_data, x='Job_Code', y='Hours', color='Hours', title="Hours Spent per Job"), use_container_width=True)
        with v2:
            worker_data = df_logs.groupby('Worker')['Hours'].sum().reset_index()
            st.plotly_chart(px.pie(worker_data, values='Hours', names='Worker', hole=0.4, title="Worker Contribution"), use_container_width=True)
