    ...
    if not df_plan.empty:
        ...
        for index, row in df_plan.iterrows():
            ...
            with st.container(border=True):
                # Defining UI elements first for reactive logic
                ...
                # --- DATE LOGIC: CALENDAR DAYS ONLY ---
                total_rem_days = int(rem_gates_count * new_limit)
                est_completion_date = (datetime.now(IST) + timedelta(days=total_rem_days)).strftime("%d %b %Y")
                ...
                # Update: Displaying Estimated Completion Date
                c4.metric("Estimated Completion Date", est_completion_date, delta=f"{total_rem_days} Days Rem.")
