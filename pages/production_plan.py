...
def get_master_data():
    try:
        # Pulling all columns (*) ensures new date columns are included
...
with tab_plan:
    ...
    if target_job != "-- Select --":
        ...
        if not proj_match.empty:
            ...
            # Dialog for editing dates (Defined before trigger)
            @st.dialog("Update Delivery Schedule")
            def update_dates_ui(job, current_po, current_rev):
                new_po = st.date_input("Original PO Date", value=current_po if current_po else date.today())
                new_rev = st.date_input("Revised Date", value=current_rev if current_rev else (current_po if current_po else date.today()))
            ...
            if 'po_delivery_date' in p_data:
                with st.container(border=True):
                    ...
                    po_dt = pd.to_datetime(p_data['po_delivery_date']).date() if pd.notnull(p_data['po_delivery_date']) else None
                    rev_dt = pd.to_datetime(p_data['revised_delivery_date']).date() if pd.notnull(p_data.get('revised_delivery_date')) else None
        ...
        if not current_job_steps.empty:
            ...
            if not valid_dates.empty:
                ...
                days_left_plan = (edd - date.today()).days
                st.info(f"📅 **Projected Completion (EDD): {edd.strftime('%d %b %Y')}** ({days_left_plan} days remaining)")
...
with tab_analytics:
    ...
    if not df_logs.empty:
        try:
            df_logs['created_at_dt'] = pd.to_datetime(df_logs['created_at'], utc=True, format='ISO8601').dt.tz_convert(IST)
            ...
            if len(d_range) == 2:
                ...
                if not report_df.empty:
                    ...
                    st.bar_chart(report_df.groupby('Job_Code')['Hours'].sum(), color="#1565C0")
                    ...
                    with col_right:
                        ...
                        st.dataframe(report_df.groupby('Job_Code')['Hours'].sum().reset_index(), use_container_width=True, hide_index=True)

                    full_csv = report_df.to_csv(index=False).encode('utf-8')
                    st.download_button("📥 Download Detailed Report (CSV)", data=full_csv, file_name="production_report.csv", mime='text/csv')
        except Exception as e:
