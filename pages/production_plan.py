...
# --- TAB 1: PRODUCTION PLANNING (Metric-Driven) ---
with tab_plan:
    if not df_plan.empty:
        for p_idx, row in df_plan.iterrows():
            ...
            with st.container(border=True):
                ...
                # SHOP FLOOR ENTRY
                st.markdown("**👷 Worker Output Log**")
                w1, w2, w3, w4 = st.columns([1.5, 1, 1.5, 1])
                worker = w1.text_input("Worker Name", key=f"wk_in_{p_idx}_{db_id}")
                qty = w2.number_input("Qty Done", min_value=0, step=1, key=f"qty_in_{p_idx}_{db_id}")
                new_gate = w3.selectbox("Move to Gate", universal_stages, index=prog_idx, key=f"gt_in_{p_idx}_{db_id}")
                ...
                with st.expander("➕ Request Material"):
                    r1, r2, r3 = st.columns([3, 2, 1])
                    req_item = r1.text_input("Item Spec", key=f"ri_in_{p_idx}_{db_id}")
                    req_qty = r2.text_input("Qty", key=f"rq_in_{p_idx}_{db_id}")
                    if r3.button("Request", key=f"rb_in_{p_idx}_{db_id}", use_container_width=True):
                        if req_item:
                            conn.table("purchase_orders").insert({"job_no": job_id, "item_name": f"SHOP: {req_item}", "specs": req_qty, "status": "Urgent"}).execute()
                            st.rerun()

# --- TAB 2: PURCHASE FULFILLMENT (Vertical Stacked) ---
with tab_fulfillment:
    if not df_plan.empty:
        for p_idx, p_row in df_plan.iterrows():
            ...
            with st.expander(f"📦 Job: {job_no} | {p_row.get('client_name')}", expanded=True):
                st.markdown('<div class="section-header">🚩 Anchor Procurement Notes</div>', unsafe_allow_html=True)
                st.write(p_row.get('critical_materials', 'None'))
                
                if not job_items.empty:
                    for i, i_row in job_items.reset_index().iterrows():
                        k = f"full_{p_idx}_{i_row['id']}_{i}"
                        with st.container(border=True):
                            ...
                            i_reply = ic2.text_area("Reply", value=i_row.get('purchase_reply', "") or "", key=f"rep_{k}", height=68, label_visibility="collapsed")
                            i_stat = ic3.selectbox("Stat", ["Triggered", "Ordered", "Received"], key=f"st_{k}", label_visibility="collapsed")
...
# --- TAB 3: PERFORMANCE REPORTS (KeyError Fix Applied) ---
with tab_reports:
    st.subheader("📊 Output Analytics")
    # THE FIX: Check if columns exist before grouping to avoid KeyError
    if not df_logs.empty and 'job_no' in df_logs.columns and 'worker_name' in df_logs.columns:
        c1, c2 = st.columns(2)
        
        # Worker Output Chart
        worker_data = df_logs.groupby('worker_name')['qty'].sum().reset_index()
        c1.plotly_chart(px.bar(worker_data, x='worker_name', y='qty', title="Worker Performance"), use_container_width=True)
        
        # Job Output Chart
        job_data = df_logs.groupby('job_no')['qty'].sum().reset_index()
        c2.plotly_chart(px.bar(job_data, x='job_no', y='qty', title="Job Completion Units"), use_container_width=True)
        
        st.dataframe(df_logs, use_container_width=True)
    else:
        st.info("No production logs found. Please enter work in the Planning tab.")

# --- TAB 4: MASTERS ---
with tab_masters:
    m1, m2 = st.columns(2)
    m1.dataframe(df_gates, use_container_width=True)
    new_gate = m2.text_input("New Gate")
    if m2.button("Add"):
