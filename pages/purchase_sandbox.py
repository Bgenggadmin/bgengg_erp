...
def get_full_purchase_data():
    try:
        ...
        if not df_all_proj.empty:
            ...
            mask = (df_all_proj['status'] == "Won") | (df_all_proj['job_no'].astype(str).str.upper().isin(active_item_jobs)) | (df_all_proj.get('purchase_trigger') == True)
...
# CRITICAL FIX: Calling the function to define df_p and df_items
...
if not df_items.empty:
    ...
    if not pending_items.empty:
        ...
        if critical_count > 0:
            st.error(f"🚨 **Productivity Alert:** {critical_count} items are Critical (>48h).")
        ...
        with st.expander(header_label, expanded=job_has_critical):
            ...
            with st.container(border=True):
                for i, i_row in active_items.sort_values('id').iterrows():
                    ...
                    with ic1:
                        if i_row['is_prod']:
                            st.markdown('<span class="tag-prod">🏗️ PRODUCTION</span>', unsafe_allow_html=True)
                        else:
                    ...
                    i_stat = ic3.selectbox("Status", ["Triggered", "Sourcing", "Ordered", "Received"], index=["Triggered", "Sourcing", "Ordered", "Received"].index(i_row['status']) if i_row['status'] in ["Triggered", "Sourcing", "Ordered", "Received"] else 0, key=f"st_{i_row['id']}", label_visibility="collapsed")
